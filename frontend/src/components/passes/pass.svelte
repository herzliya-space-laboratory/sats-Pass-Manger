...
<tr class="flex w-full mb-4">
    <td class={"px-6 pt-6 whitespace-no-wrap" + fieldWidthClass}>
        <div class="text-xl text-center leading-5 text-white">{pass.Satellite.name} </div>
    </td>
    ...
    <td class={"px-6 pt-6 whitespace-no-wrap text-xl text-center leading-5 font-medium" + fieldWidthClass}>
        ...
        <a href={passPageUrl} type="button"
         class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm leading-5 font-medium rounded-md text-white bg-black hover:text-gray-500 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 active:text-gray-800 active:bg-gray-50 transition duration-150 ease-in-out">
